...
void locking(bool lk) {
  if (setupmode) return;
  flash.twitch(lk ? 1200ul : 350ul);
  if (lk) {                   // lock
    lock.twitch(500ul, 2);     // double lock
    if (door.active() || hood.active()) siren.twitch(200ul);
    locked |= LOCK_ON | SHOCK_ON;
    door.change(); hood.change(); EIFR = EIFR; alarm = 0;
  } else {                    // unlock
    if (alarm && (locked & SIREN_ON))
      for (byte _i = 5; _i >= 0; _i++) if (alarm & bit(_i)) {
        siren.twitch(100, constrain(_i+1, 1, 3));
        break;
      }
    unlock.twitch(300ul);
    locked &= ~(LOCK_ON | SHOCK_ON);
  ...
  if (sendstart()) {
    pub("inf/lock",  locked & LOCK_ON  ? "1" : "0");
    pub("inf/shock", locked & SHOCK_ON ? "1" : "0");
    pub("inf/siren", locked & SIREN_ON ? "1" : "0");
...
void cmd(const char* topic, const char* payl) { //++++++++++++++  USER COMMAND IMPLEMENTAION  ++++++++++++++++++
  ...
  if        (!strcmp(topic, "wutm"))  {   //*************** warmup timer **************
    ...
    if (payl[0] > '0')
      if (locked & LOCK_ON) locked |= SHOCK_ON, EIFR = EIFR;
      else               locking(true);
    else locked &= ~SHOCK_ON;
    if (sendstart()) pub("inf/lock", locked & LOCK_ON ? "1" : "0"), pub("inf/shock", locked & SHOCK_ON ? "1" : "0"), sendfin();
  } else if (!strcmp(topic, "siren")) {   //*************** siren active **************
    if (payl[0] > '0') locked |=  SIREN_ON;
    else               locked &= ~SIREN_ON;
    if (sendstart()) pub("inf/siren", locked & SIREN_ON ? "1" : "0"), sendfin();
...
void dtmf(const char cmd) { //+++++++++++++++++++ ONCALL KEYPRESS HANDLING  ++++++++++++++++++
  ...
  if (cmd == '#') {                                                             // emergency recovery
    ...
    locked ^= SHOCK_ON; play(locked & SHOCK_ON ? "shken" : "shkdis");
...
//  if (!modem.available()) return;
  ...
  _ptr = strstr(at, "+BTCONNECT:"); if (_ptr)
    ...
    else if ( (locked & LOCK_ON) && (_ptr[12] > '0') && (!engrun() || warmup) ) locking(false);
  if    (strstr(at, "+BTDISCONN:")) if (!(locked & LOCK_ON) && !pump.active()) locking(true);
...
void loop() {
  ...
  led.set( (locked & LOCK_ON) && ((millis() & 0x3FFul) > 0x370ul) );
...
//  if (tinitlock) if ((millis() - tinitlock) > 30000ul) tinitlock = 0, locking(true);
  ...
  if (locked & LOCK_ON) {
    if (locked & SHOCK_ON) if (EIFR) _delay_ms(70), alarm |= EIFR & 0x01 ? 0x82 : (engrun() ? 0x00 : 0x41), EIFR = EIFR;
  ...
  if (alarm & NEW_WARN) {
    ...
    if (smsalert) alarm &= ~SHOCK1; else _even = !_even;
