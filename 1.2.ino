...
void starting() {
  ...
  while ( ((millis() - _tinit) < 3000ul) && (vbatt.value() < (_vinit - 1.6f)) );  // spinup engine - max 3.0sec
...
//  if (tinitlock) if ((millis() - tinitlock) > 30000ul) tinitlock = 0, locking(true);
  ...
  } if (!(alarm & 0x20)) if  ( (temps[DIF][PCB] > 5) || (temps[CUR][PCB] > 70) || (!engrun() && ((temps[DIF][OUTS] > 3) || (temps[DIF][VEH] > 3))) ) ign.set(false), alarm |= 0xA0;
  if (alarm > 0x81) {
