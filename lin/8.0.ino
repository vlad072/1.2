...
void starting() {
  uint32_t _tstart = (uint32_t)tstart();
...
void btspp(const char* frame, uint8_t len) { //+++++++++++++++++++ BLUETOOTH SETUP THROUGH SPP  ++++++++++++++++++
  ...
  if        (!strncmp(frame, "srv", 3)) {
    ...
    if (isdigit(_paramptr[0])) 
      batt.reset(BATT_PIN, 1.0f),
      kbatt(atoi(_paramptr)/(float)batt.value()),
      batt.reset(BATT_PIN, kbatt()),
      btprintln(F(" ok"));
    else
      btprint(F(" ")), btprintln(batt.value());
  } else if (!strncmp(frame, "gear", 4)) {                          // =========== teach gear ============
    gear.reset(NEUTRAL_PIN, 1.0f);
    if (_paramlen && (_paramptr[0] == '?')) {
      btprint(F(" ")); btprintln(gear.value());
    } else {
      int16_t _lvl = gear.value();
      btprint(F(" shift the handle... ")); wdt_reset();
      for ( uint32_t _t = millis(); !timeover(_t, 3000ul); ) if ( abs(gear.value() - _lvl) > 35 ) {
        kgear(1.0f/(gear.value() + _lvl)); gear.reset(NEUTRAL_PIN, kgear());
        btprintln(F("ok")); return;
      }
      btprintln(F("fail!"));
    }
    gear.reset(NEUTRAL_PIN, kgear());
  } else if (!strncmp(frame, "tstart", 4)) {                        // ========== starter time ===========
    if (isdigit(_paramptr[0]))
      tstart(atoi(_paramptr)), btprintln(tstart() ? F(" ok") : F(" fail!"));
    else
