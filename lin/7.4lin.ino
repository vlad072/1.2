...
void btspp(const char* frame, uint8_t len) { //+++++++++++++++++++ BLUETOOTH SETUP THROUGH SPP  ++++++++++++++++++
  ...
  if        (!strncmp(frame, "srv", 3)) {
    ...
    btprintln(F("press... "));
    uint8_t _i = 0; uint8_t _in = 0;
    for ( uint32_t _t = millis(); !timeover(_t, 5000ul); ) if (lin.available() > 0) {
      _in = lin.read(); _t = millis() - 4950ul;
      if (_i == 4) {
        key(_in);
        modem.println(F("AT+BTSPPSEND")); if (waitresp("> ")) hexprint(modem, key(), 1), modem.println(F(" ok")), modem.write(0x1A), waitresp();
        return;
      }
      _i++;
    }
