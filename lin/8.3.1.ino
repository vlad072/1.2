...
Adc    batt(BATT_PIN, kbatt()), gear(NEUTRAL_PIN, kgear());
...
bool waitresp(char* str = NULL, uint32_t timeout = DEF_TIMEOUT) {
  ...
  for (uint32_t _t = millis(); !timeover(_t, timeout);) {
    ...
    _t = millis(); timeout = 20ul; _c = modem.read();
    if (!str) continue;
    _i = (str[_i] == _c) ? _i+1 : 0;
...
void pub(const char* topic, const char* payl, uint8_t retn = 0) {
  ...
  modem.print(pref()); modem.print(F("inf/")); modem.print(topic);
  modem.print(payl);
}
...
void sub() {
  ...
  modem.print(pref()); modem.print(F("cmd/#"));   //topic
...
void brokercon () {
  ...
  modem.write((byte)0x00); modem.write(0x06);  modem.print(F("MQIsdp")); modem.write(3);
  ...
  modem.write((byte)0x00); modem.write(strlen(pref())+10); modem.print(pref()); modem.print(F("inf/online"));  // will topic
...
void arming(bool arm) {
  ...
  if (arm) {                   // ================== lock ======================
    if (_op) siren.twitch(100ul, 3); else if (secure & 0x40) siren.set(1), _delay_ms(10ul), siren.set(0);
    secure |= 0x80;
    ...
    if (secure & 0x40) {
      if (alarmcash) {
        ...
        siren.set(1); _delay_ms(10ul); siren.set(0);
      }
    }
    secure &= ~0x80;
  }                           // ===============================================
...
void linsend(uint8_t sign) {
  while ( (linbuf[1] = (random(0xFF) & 7)) == 0 );
  linbuf[2] = hash(linbuf[1], sign);
  linbuf[3] = linbuf[1] ^ linbuf[2];
  linbuf[4] = key(TAIL_KEY);
  lin.write(key(HEAD_KEY)); _delay_us(39110ul);
  lin.write(linbuf+1, sizeof(linbuf)-1);
  lin.flushOutput(); _delay_ms(10ul); lin.flushInput();
}
void linrecv() {
  uint8_t _in = 0; linsz = 0;
  for ( uint32_t _t = millis(); !timeover(_t, 50ul); ) if (lin.available() > 0) {
    _in = lin.read();
    if (linsz < sizeof(linbuf)) linbuf[linsz] = _in;
    linsz++; _t = millis();
  }
}
void exec(const char* topic, const char* payl) { //++++++++++++++  USER COMMAND IMPLEMENTAION  ++++++++++++++++++
  if        (!strcmp(topic, "wutm"))  {   //*************** warmup timer **************
    ...
    dvrmode = atoi(payl);
  } else if (!strcmp(topic, "keyrj")) {   //************ reject native key ************
    keyreject = payl[0] > '0';
  } else if (!strcmp(topic, "btpwr")) {   //*************** blue power on/off ********
    bool _pw = atoi(payl) > 0;
    modem.print(F("AT+BTPOWER=")); modem.println(_pw, DEC);
    wdt_reset(); waitresp(NULL, 3900ul); wdt_reset();
    if (_pw) {
      modem.println(F("AT+BTVIS=0"));
      if (!waitresp("OK\r\n")) modem.println(F("AT+BTPOWER=0")), waitresp(NULL, 2900ul);
      wdt_reset();
    }
    setupmode = 0; eepage = 0; led.set(setupmode);
  } else if (!strcmp(topic, "btansw")) {   //************ hfp auto-answer *************
    btansw = payl[0] > '0';
  } else if (!strcmp(topic, "setup")) {    //************** enter setup ***************
    modem.print(F("AT+BTVIS=")); modem.println(payl);
    if (waitresp("OK\r\n")) setupmode = (payl[0] > '0'), eepage = 0, led.set(setupmode);
...
#define btprint(content) modem.println(F("AT+BTSPPSEND")); if (waitresp("> ")) modem.print(content), modem.write(0x1A), waitresp()
...
void btspp(const char* frame, uint8_t len) { //+++++++++++++++++++ BLUETOOTH SETUP THROUGH SPP  ++++++++++++++++++
  ...
  if        (!strncmp(frame, "srv", 3)) {
    if (_paramptr[0] == '?') {
      modem.println(F("AT+BTSPPSEND")); if (waitresp("> ")) {
        modem.println();
        modem.print(F("srv=")); modem.println(broker());
        modem.print(F("usr=")); modem.println(user());
        modem.print(F("pwd=")); modem.println(pass());
        modem.print(F("pref=")); modem.println(pref());
        modem.write(0x1A); waitresp();
      }
    } else {
  ...
  } else if (!strncmp(frame, "key", 3)) {
    btprint(F(" any press... ")); wdt_reset();
    for ( uint32_t _t = millis(); !timeover(_t, 3000ul); ) if (lin.available() > 0) {
      linrecv();
      if ( (linsz == 5) && (linbuf[1]^linbuf[2]^linbuf[3] == 0) ) {
        modem.println(F("AT+BTSPPSEND")); if (waitresp("> ")) hexprint(modem, linbuf, linsz), modem.println(F(" ok")), modem.write(0x1A), waitresp();
        key(HEAD_KEY, linbuf[0]); key(TAIL_KEY, linbuf[4]);
        return;
      } else break;
    }
    btprintln(F("fail!"));
  } else if (!strncmp(frame, "batt", 4)) {                          // ========= teach battery ===========
    if (isdigit(_paramptr[0])) 
      batt.reset(1.0f),
      kbatt(atoi(_paramptr)/(float)batt.value()),
      batt.reset(kbatt()),
      btprintln(F(" ok"));
    else
      btprint(F(" ")), btprintln(batt.value());
  } else if (!strncmp(frame, "gear", 4)) {                          // =========== teach gear ============
    gear.reset(1.0f);
    if (_paramlen && (_paramptr[0] == '?')) {
      btprint(F(" ")); btprintln(gear.value());
    } else {
      int16_t _lvl1 = gear.value(); int16_t _lvl2 = _lvl1;
      btprint(F(" shift the handle... ")); wdt_reset();
      for ( uint32_t _t = millis(); !timeover(_t, 3000ul); ) if ( abs(gear.value() - _lvl1) > 35 ) {
        _delay_ms(200ul); _lvl2 = gear.value(); if (abs(_lvl1 - _lvl2) < 35) break;
        kgear(1.0f/(_lvl1 + _lvl2)); gear.reset(kgear());
        btprint((_lvl1 + _lvl2)/2); btprintln(F(" ok")); return;
      }
      btprintln(F("fail!"));
    }
    gear.reset(kgear());
  } else if (!strncmp(frame, "tstart", 4)) {                        // ========== starter time ===========
    if (isdigit(_paramptr[0]))
      tstart(atol(_paramptr)), btprintln(tstart() ? F(" ok") : F(" fail!"));
    else
      btprint(F(" ")), btprintln(tstart()); 
    ...
    if ( (_idx >= PCB) && (_idx <= VEH) ) {
      ...
      modem.println(F("AT+BTSPPSEND")); if (waitresp("> ")) {
        ...
        ds.reset_search(); modem.println(F("scan:"));
    ...
    modem.println(F("AT+BTSPPSEND")); if (waitresp("> ")) {
      ...
      modem.println(F("batt=<batt voltage (mV)>, ?-info"));
      modem.println(F("tstart=<starter time (ms)>, ?-info"));
      modem.println(F("gear : gear threshold teach, ?-info"));
      modem.println(F("sens=<N> : 0-pcb,1-eng,2-outs,3-vehcl,?-info"));
      modem.println(F("btpin=<NNNN> : bt devices detach & upd pin"));
      modem.println(F("key : keychain teach"));
...
void dtmf(const char dtone, bool lon) { //+++++++++++++++++++ ONCALL KEYPRESS HANDLING  ++++++++++++++++++
  stoptrack();
  if ( ((dtone == '#') || (dtone == '0')) && lon ) {                               // emergency recovery
    wdt_disable(); modem.setTimeout(5000);
    play((dtone == '#') ? "recov" : "reboot"), modem.find("+CREC: 0");
    modem.println(F("ATH")); waitresp();
    if (sendstart()) modem.write(0xE0), modem.write((byte)0x00), sendfin(); // disconnect
    if (dtone == '#') modemreconf();
    reboot();
  } else if (dtone == '1') {                                                      // help notice
    play("help");
  } else if (dtone == '2') {                                                      // keep broker connection
    keepconn = !keepconn; play(keepconn ? "keep1" : "keep0");
  } else if (dtone == '3') {
    if (secure & 0x03) secure &= ~0x03; else secure |= 0x03;                      // shock sensor enble
    play(secure & 0x03 ? "shk1" : "shk0");
  } else if (dtone == '4') {                                                      // bt on/off
    ...
    play(_btpw ? "bt1" : "bt0");
  } else if (dtone == '5') {                                                      // dvr run to 1min
    ...
    if (lon) {                                                                    // setup mode
      if (setupmode) {
        ...
        modem.println(F("AT+BTPOWER=1"));
        wdt_reset(); waitresp(NULL, 3900ul); wdt_reset();
        modem.println(F("AT+BTVIS=1"));
        waitresp(); setupmode = 1;
        //if (waitresp("OK\r\n")) setupmode = 1;
      }
...
void dsupdate() {
  ...
  if ( (secure & 0xA0) == 0xA0 ) {                                             // overheats:
    int8_t _do = 1; int8_t _dv = 1; uint32_t _enghold = (millis()-tengrun)/60000;
    if      (_enghold < 10)  _do = 4, _dv = 2; 
    else if (_enghold < 45)  _do = 3;
    else if (_enghold < 120) _do = 2;
    if ( (temps[DIF][PCB]  > 1)  || (temps[CUR][PCB] > 70) ||  //   pcb
         (temps[DIF][OUT] > _do) || (temps[CUR][OUT] > 65) ||  //   outside
         (temps[DIF][VEH] > _dv) || (temps[CUR][VEH] > 80) )   //   vehicle
...
void linhandling() {
  linrecv();
  if (linsz != 5) return;
  if (linbuf[1] ^ linbuf[2] ^ linbuf[3]) return;
  if (linbuf[0] != key(HEAD_KEY)) return;
  if (linbuf[4] != key(TAIL_KEY)) return;
//  if (_buf[1] & 7 == 0) return;
  if (linbuf[2] == hash(linbuf[1], OPEN_SIGN))  strcpy(msg, "KEY"), arming(0);
  if (linbuf[2] == hash(linbuf[1], CLOSE_SIGN)) strcpy(msg, "KEY"), arming(1);
}
...
void loop() {
  ...
  if (lin.available() > 0)
    if (keyreject)
      for (uint32_t _t = millis(); !timeover(_t, 50);) lin.write((byte)0), lin.flushOutput(), lin.flushInput(); 
    else
      linhandling();
  led.set( (secure & 0x80) && ((millis() & 0x3FFul) > 0x370ul) );
  if (dvrmode & 0x08) if (batt.value() > 11700) dvr.twitch(5000ul);
  if (pump.active()) {
    tengrun = millis();
  ...
  if (warmup) {
    warmup = pump.active() && !gear.value() && (warmtemp > temps[CUR][ENG]) && (warmtimer > 0);   
  ...
  if (secure & 0x80) {
    ...
    if (batt.value() < lowbatt) {
      strcpy(msg, "LOW BATT: "); dtostrf(batt.value()/1000.0f, 5, 2, msg+10); notify();
      lowbatt -= 250;
    } else {
      if (batt.value() > 12100) lowbatt = 12000;
    }
  } else {
  ...
  if (alarm & 0x40) {
    ...
    if (callstate < 0x04){
      notify("BANG! (>_<)", PUSH+SMS), alarm &= 0xFE;
