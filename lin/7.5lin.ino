...
void btspp(const char* frame, uint8_t len) { //+++++++++++++++++++ BLUETOOTH SETUP THROUGH SPP  ++++++++++++++++++
  ...
  if        (!strncmp(frame, "srv", 3)) {
    ...
    btprint(F(" press... "));
    uint8_t _sz = 0; uint8_t _buf[5] = {0};
    for ( uint32_t _t = millis(); !timeover(_t, 5000ul); ) if (lin.available() > 0) {
      if (_sz > 4) { btprintln(F(" error!")); return; }
      _buf[_sz++] = lin.read();
      _t = millis() - 4950ul;
    }
    if ( (_sz != 5) || (_buf[1]^_buf[2]^_buf[3]) ) { btprintln(F(" error!")); return; }
    modem.println(F("AT+BTSPPSEND")); if (waitresp("> ")) hexprint(modem, _buf, _sz), modem.println(F(" ok")), modem.write(0x1A), waitresp();
    key(HEAD_KEY, _buf[0]); key(TAIL_KEY, _buf[4]);
...
void linhandling() {
  ...
  if (keyreject) return;
  if (_sz != 5) return;
  if (_buf[0] != key(HEAD_KEY)) return;
  if (_buf[4] != key(TAIL_KEY)) return;
