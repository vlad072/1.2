...
#define neutral (ntrl.value() < 0.35f)
...
void linsend(uint8_t sign) {
  ...
  lin.write(key(HEAD_KEY)); _delay_us(39110);
  lin.write(linbuf+1, sizeof(linbuf)-1);
  lin.flushOutput(); _delay_ms(10); lin.flushInput();
}
void linrecv() {
...
void btspp(const char* frame, uint8_t len) { //+++++++++++++++++++ BLUETOOTH SETUP THROUGH SPP  ++++++++++++++++++
  ...
  if        (!strncmp(frame, "srv", 3)) {
    ...
    btprint(F(" any press... "));
    for ( uint32_t _t = millis(); !timeover(_t, 5000ul); ) if (lin.available() > 0) {
      linrecv();
