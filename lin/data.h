...
#define PAGESZ            0x40
//==================== temps indexes ====================
enum { PCB, ENG, OUT, VEH }; // dallas order
enum { CUR, DIF };            // curent, difference
//======================= consts ========================
const char* cid     = "BB7";            // device signature
const char* fwver   = "8.3.1";            // firmware version
//===================== variables ======================
char     at[128]    =  "";      // AT buffer
char     msg[32]    =  "";      // message buffer
uint8_t  linbuf[5]  =  {0};     // lin buffer
uint8_t  linsz      =  0;       // lin buffer size
...
uint8_t  dvrmode    =  0;       // recording dvr 1 = drive, 2 = parking, 3 = both
...
bool     keyreject  =  0;       // native key reject
int16_t  lowbatt    =  12000;   // low battery warning lvl
...
uint8_t key(uint8_t idx, uint8_t k = 0) {                                    // 140      ======= key =======
  if (k) eeprom_update_byte(0x140+idx, k);
  return eeprom_read_byte(0x140+idx);
}

float kbatt(float k = 0.0f) {                                                // 180      ===== settings ====
  if (k != 0.0f) eeprom_update_float(0x180, k);
  return eeprom_read_float(0x180);
}
float kgear(float k = 0.0f) {
  if (k != 0.0f) eeprom_update_float(0x184, k);
  return eeprom_read_float(0x184);
}
uint32_t tstart(uint32_t t = 0) {
  if (t != 0) eeprom_update_dword(0x188, t);
  return eeprom_read_dword(0x188);  
}
