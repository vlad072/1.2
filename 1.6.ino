...
//  if (tinitlock) if ((millis() - tinitlock) > 30000ul) tinitlock = 0, locking(true);
  ...
  if (locked & 0x80) {
    ...
    if (fuelpump.change() == 1) alarm |= 0x90;
                              ...
                              (temps[DIF][VEH]  > (engrun() ? 5 : 2)) || (temps[CUR][VEH]  > 75) || ((temps[CUR][VEH] - temps[CUR][OUTS]) > 35) )
                                ign.set(false), alarm |= 0xA0;
  if (alarm & 0xC0) if (sendstart()) pubalarm(), sendfin();
  if (alarm & 0x80) {
    alarm &= ~0x80; dvr.run(60000ul);
    if ( celstate == 0 ) modem.println(F("ATD>\"admin\";"));
    if ( celstate < 2 ) {
