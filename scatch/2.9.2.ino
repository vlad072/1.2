...
void(*reboot)(void) = 0;
inline __attribute__((always_inline)) bool timeover(uint32_t val, uint32_t timeout) {
  return (millis() - val) >= timeout;
}
...
void play(const char* track) {
  if (celstate != 0x0C) return; dbg("play", track);
  celstate = 0x04;
...
void btspp( const char* inp, byte len ) { //+++++++++++++++++++ BLUETOOTH SETUP THROUGH SPP  ++++++++++++++++++
  ...
  if        ( !strncmp(inp, "srv", 3) && (_paramlen < 0x20) ) {
    ...
    modem.println(F("AT+BTSPPSEND")); if (modem.find("> ")) modem.println(F(" ok")), modem.write(0x1A), modem.find("SEND OK\r\n");
  } else if ( !strncmp(inp, "sens", 4) && isdigit(_paramptr[0]) ) {   // ============ sensors ==============
...
void dtmf(const char key) { //+++++++++++++++++++ ONCALL KEYPRESS HANDLING  ++++++++++++++++++
  ...
  if (key == '#') {                                                             // emergency recovery
    wdt_disable();
    if (celstate == 0x0C) modem.setTimeout(5000), play("recov"), modem.find("+CREC: 0");
  ...
  } else if (key == '0') { dbg("setup");                                        // setup mode
...
//  if (!modem.available()) return;
  ...
  if (strstr(at, "Call Ready")) celstate = 1;
  ...
  _ptr = strstr(at, "+CLCC: 1,"); if (_ptr) {
    switch (_ptr[11]) {
      case '0': siren.set(false); flash.set(false); celstate = 0x0C; play("hello"); break;
      case '2':
      case '3': celstate = 0x02; break;
      case '6':
      case '7': celstate = 0x01; tsend = tresp = millis() - 23000ul; break;
      case '4': modem.println(strstr(_ptr+17, "\"admin\"") ? F("AT+DDET=1,500,0;A") : F("ATH"));
    } t20sec = millis();
  }
  if (strstr(at, "+CREC: 0")) celstate |= 0x08;
  if (strstr(at, "+CPAS: 0")) celstate  = 0x01;
...
void loop() {
  ...
  if (timeover(t20sec, 20000ul)) {
    ...
    if ((celstate != 1) && !modem.available()) modem.println(F("AT+CPAS")); // query cellular status - freez calling state fix
  }
  if (warmup) {
  ...
  if ( (alarm & 0x3E) && (celstate == 1) )
    if ( ((nrecall > 0) && timeover(tlastcall, 120000ul)) || (alarm & 0x80) )
      modem.println(F("AT+DDET=1,500,0;D>\"admin\";")), tlastcall = millis(), nrecall--, celstate = 0;
  if (alarm & 0x40) {
    dvr.twitch(30000ul);
