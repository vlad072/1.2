...
void(*reboot)(void) = 0;
inline __attribute__((always_inline)) bool timeover(uint32_t var, uint32_t timeout) {
  ...
  if        ( !strncmp(inp, "srv", 3) && (_paramlen < 0x20) ) {
    ...
    modem.println(F("AT+BTSPPSEND")); if (modem.find("> ")) {
      ...
      if (_found) {
        ...
        for (byte _i = 0; _i < 8; _i++) { if (_sid[_i] < 0x10) modem.write('0'); modem.print(_sid[_i], HEX); }
