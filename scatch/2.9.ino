...
void publocate(const char* inp) {
  char _link[24] = "85.000000, -135.000000"; // atctic :)
  if (inp[0] == '0') {
...
void loop() {
  ...
  if (keepconn && !celstate) {
    if ((millis() - tsend) > 27000ul) if (sendstart()) { modem.write(0xC0); modem.write(byte(0x00)); if (sendfin()) tsend = millis(); } // ping
    if ((millis() - tresp) > 60000ul)  modemshut();  // broker isn't responding!
  }
...
//      if (modem.find("OK\r\n")) celstate = 0x01;
  ...
  if (alarm & 0x40) {
    dvr.twitch(30000ul);
    if ((millis() - tlastmsg) > (smsnotify ? 5000ul : 500ul)) notify("BANG! (>_<)", smsnotify ? SMS : PUSH+SMS), tlastmsg = millis();
