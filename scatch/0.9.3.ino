...
Adc    vbatt(BATT_PIN, 59.63f), ntrl(NEUTRAL_PIN);
...
bool neutral() {
  return ntrl.value() < 50.0f;
}
bool engrun() {
  return fuelpump.active() || (vbatt.value() > 13.5f);
}
bool drive() {
  return engrun() && !neutral();
}
void starting() {
  if ( engrun() || !neutral() || ((warmtemp - temps[CUR][ENG]) < 5) || (vbatt.value() < 11.0f) ) return; dbg("==start==");
  if (sendstart()) pubf("inf/wutm", warmtimer, 0), pubf("inf/wutp", warmtemp, 0), sendfin();
//  float _vinit = 0.0f; uint32_t _tinit = 0ul;
  ...
  while ( ((millis() - _tinit) < 2000ul) && (vbatt.value() < (_vinit - 1.5f)) );  // spinup engine - max 2.0sec
  starter.set(false); delay(100); if (!starter.active()) goto _ok;
  _err: dbg("ERR lbl"); ign.set(false); return;
  _ok:  dbg("OK lbl"); delay(5000);  
  warmup = engrun();
//  debug.print(F("dV=")); debug.println((vbatt.value() - _vinit), 2);
//  debug.print(F("fp.active=")); debug.println(fuelpump)
  if (!warmup) ign.set(false);
  fuelpump.change(); EIFR = EIFR; tresp = millis(); 
}
...
bool sendfin() {
  ...
  /*if (_ret) */tsend = millis(); dbg(_ret ? "sendOK" : "sendERR");
  return _ret;
}
...
void cmd(const char* topic, const char* payl) { //++++++++++++++  USER COMMAND IMPLEMENTAION  ++++++++++++++++++
  ...
  if        (!strcmp(topic, "wutm"))  {   //*************** warmup timer **************
  ...
  } else if (!strcmp(topic, "sln")) {   //*************** silent mode ***************
    if (payl[0] > '0') locked &= ~2;
    else               locked |=  2;
    if (sendstart()) pub("inf/sln", locked & 2 ? "0" : "1"), sendfin();
  } else if (!strcmp(topic, "sms")) {      //*************** sms notify kick warning ***
    smsalert = payl[0] > '0';
    ...
    modem.println(F("AT+BTSTATUS?")); // trouble!!! H%(*UQUM⸮ instead +BTSTATUS:
  ...
  } else if (!strcmp(topic, "alarm")) {    //*************** clear alarms **************
    switch (payl[0]) {    // 1=clear,2=mute,3=raise
      case '1': alarm = 0; if (sendstart()) pub("inf/alarm", "0"), sendfin();
      case '2': siren.set(false); flash.set(false); break;
      case '3': if (locked & 2) siren.run(1000ul); flash.run(3000ul);
    }
...
void ipd( const byte* frame, byte len = 1 ) { //++++++++++++++  PROCESSING DATA FROM BROKER  ++++++++++++++++++
  ...
  for (byte _idx = 0; _idx < len;) {
    if ( (frame[_idx] == 0xD0) && (frame[_idx+1] == 0x00) ) {            // pingresp d0 00
      dbg("pingresp");
    } else  if (frame[_idx] == 0x20) {                                   // connak 20 02 00 00
      ...
      dbg("suback");
    } else if (frame[_idx] == 0x30) {                                    // pub recv !!sometime 31 10 00 0c
...
void btspp( const char* msg, byte len ) { //+++++++++++++++++++ BLUETOOTH SETUP THROUGH SPP  ++++++++++++++++++
  ...
  if        ( !strncmp(msg, "srv", 3) && (_paramlen < 0x20) ) {
    ...
    modem.println(F("AT+BTSPPSEND")); if (modem.find("> ")) modem.print(F(" ok\r\n")), modem.write(0x1A);
...
void dtmf(const char cmd) { //+++++++++++++++++++ ONCALL KEYPRESS HANDLING  ++++++++++++++++++
  ...
  if (cmd == '#') {                                                             // emergency recovery
    ...
    keepconn = !keepconn; if (celstate == 6) play(keepconn ? "keep" : "nkeep");
  } else if (cmd == '3') {                                                      // shock sensor enble
    locked ^= 1; if (celstate == 6) play(locked & 1 ? "shken" : "shkdis");
    ...
    if (celstate == 6) play(setupmode ? "setup" : "error");
    modem.setTimeout(DEF_TIMEOUT);
  } else {
    if (celstate == 6) play("error"); // unknown command
  }    
}
...
//  if (!modem.available()) return;
  ...
  for (uint32_t _trecv = millis(); (millis() - _trecv) < 10ul;) { // CHECK!!!
  ...
  if (strstr(at, "RING")) modem.println(F("AT+CLIP=1")); // CHECK!!!
  ...
  _ptr = strstr(at, "+BTSTATUS:");  if (_ptr) {
    ...
    if (sendstart()) pub("inf/btpow", _ptr[11] > '0' ? "1" : "0"), pubf("inf/btpair", _pair, 0), pubf("inf/btcon", _conn, 0), pub(willt, "1"), sendfin();
  }
  ...
  _ptr = strstr(at, "+CPBF:");  if (_ptr) {                                             // recv ph.num "admin" then send sms // CHECK!!!
    modem.println("AT+CMGF=1"); if (modem.find("OK\r\n")) {
      ...
      while (_nptr[0]) {
        modem.write(_nptr++[0]); if (_nptr[0] == '"') break;        // CHECK!!!
      } modem.println('"');
...
void dsupdate() {
  for( byte _i = 0; _i < 4; _i++ ) ds.reset(), ds.select(sid(_i)), ds.write(0x44, 1); _delay_ms(150);
  for( byte _i = 0; _i < 4; _i++ ) {
...
void setup() {
  ...
  DDRD &= 0xF3; PORTD |= 0x0C; EICRA = 0x0A; EIFR = EIFR;    // D2,D3: input, pullup, falling, clear
...
//  if (debug.available()) { while (debug.available()) debug.read(); starting(); }
  ...
  if (starter.active()) dbg("st-r.activate"), ign.set(false); // CHECK
  if (setupmode) {
    if (EIFR) {
      delay(100);
...
//    if (sendstart()) pub("notify", "broker isn't responding!"), sendfin();
  ...
  if (tbtvis) if ((millis() - tbtvis) > 30000ul) tbtvis = 0, modem.println(F("AT+BTVIS=0"));
//  if (tinitlock) if ((millis() - tinitlock) > 30000ul) tinitlock = 0, locking(true);
  if (warmup) {
    bool _refresh = false;
    if ((millis() - t1min) > 60000ul) { dbg("t1min"); t1min = millis(); if (warmtimer > 0) warmtimer--; _refresh = true;}
    warmup = neutral() && (warmtemp > temps[CUR][ENG]) && (warmtimer > 0);
    if (!neutral()) dbg("gear.set");    
    if (!warmup) dbg("warm.over"), ign.set(false);
    if (!warmup || _refresh) {
      dbg("reftesh");
      if (sendstart()) {
        pub ("inf/warmup", warmup ?  "1" : "0");
  ...
  if (locked & 0x80) {
    if (locked & 1) if (EIFR) alarm |= EIFR & 1 ? 0x82 : 0x41, EIFR = EIFR;
    if (hood.change()     == 1) alarm |= 0x84;
    if (door.change()     == 1) alarm |= 0x88;
    if (fuelpump.change() == 1) alarm |= 0x90;
  } if (!(alarm & 0x20)) if  ( (temps[DIF][PCB] > 5) || (temps[CUR][PCB] > 70) || (!engrun() && ((temps[DIF][OUTS] > 3) || (temps[DIF][VEH] > 3))) ) alarm |= 0xA0;
  if (alarm > 0x81) {
    alarm &= ~0x80; dvr.run(60000ul);
    if ( celstate == 0 ) modem.println(F("AT+COLP=1;D>\"admin\";")), celstate = 1;
    if ( celstate < 2 ) { if (locked & 2) siren.run(15000ul); flash.run(20000ul); }
  } else  if (alarm & 0x40) {
    static bool _even = false; strcpy(notify, "KICK!"); if (_even) strcat(notify, "!");
    if (smsalert) modem.println(F("AT+CPBF=\"admin\"")), alarm &= ~1;
    else if (sendstart()) pub("notify", notify), sendfin(), _even = !_even;
         else             modem.println(F("AT+CPBF=\"admin\"")), alarm &= ~1; // request ph.num admin
    dvr.run(30000ul); if (!siren.on() && (locked & 2)) siren.run(300ul); if (!flash.on()) flash.run(2000ul);
    alarm &= ~0x40;
  }
