...
void exec(const char* topic, const char* payl) { //++++++++++++++  USER COMMAND IMPLEMENTAION  ++++++++++++++++++
  ...
  if        (!strcmp(topic, "wutm"))  {   //*************** warmup timer **************
    ...
    if (payl[0] > '0') {
      ...
      if (!modem.find("OK\r\n")) { modem.println(F("AT+BTPOWER=0")); wdt_reset(); modem.find("OK\r\n"); }
    }
...
void btspp(const char* inp, byte len) { //+++++++++++++++++++ BLUETOOTH SETUP THROUGH SPP  ++++++++++++++++++
  ...
  if        ( !strncmp(inp, "srv", 3) && (_paramlen < 0x20) ) {
    ...
    modem.println(F("AT+BTSPPSEND")); if (modem.find("> ")) modem.println((moto(_min) == _min) ? F(" ok") : F(" error")), modem.write(0x1A), modem.find("SEND OK\r\n");
  } else if (inp[0] == '?') {
    modem.println(F("AT+BTSPPSEND")); if (modem.find("> ")) {
      modem.println();
...
void dtmf(const char key, bool lon) { //+++++++++++++++++++ ONCALL KEYPRESS HANDLING  ++++++++++++++++++
  ...
  if ( ((key == '#') || (key == '0')) && lon ) {                                                    // emergency recovery
    wdt_disable();
    if (callstate == 0x0C) modem.setTimeout(5000), play((key == '#') ? "recov" : "reboot"), modem.find("+CREC: 0");
