...
bool waitresp(Stream& uart, char* str = NULL, uint32_t timeout = DEF_TIMEOUT) {
  byte _i = 0; char _c = 0;
  for (uint32_t _t = millis; !timeover(_t, timeout);) {
    if (!uart.available()) continue;
    _t = millis(); timeout = 10ul; _c = uart.read();
    if (!str) continue;
    _i = (str[_i] == _c) ? _i+1 : 0;
    if (_i >= strlen(str)) return 1;
  }
  return 0;
}
bool neutral() {
...
void modemreset() {
  dbg("reset modem...");
  modem.println(F("ATZ+CFUN=1,1")); waitresp(modem);  // user profile recovery; reboot modem 
  callstate = 0; tsend = tresp = millis();
}
void modemreconf() {
  dbg("modem reconf...");
  modem.print  (F("AT&F+IPR="));  modem.println(UART_BPS, DEC);       waitresp(modem); // usart speed
  modem.print  (F("AT+BTHOST=")); modem.println(cid);                 waitresp(modem); // blue host name
  modem.println(F("AT+CMIC=0,15;+BTPAIRCFG=1;+CLCC=1;+CMGF=1;E0&W")); waitresp(modem);
}
void modemshut() {
  modem.println(F("AT")); if (!waitresp(modem, "OK\r\n")) modem.write(0x1A), waitresp(modem);
...
void play(const char* track) {
  ...
  if (waitresp(modem, "OK\r\n")) callstate = 0x04;;
}
bool sendstart() {
  modem.println(F("AT+CIPSEND"));
  return waitresp(modem, "> ", 500ul);
}
bool sendfin() {
  modem.write(0x1A);
  return waitresp(modem, "DATA ACCEPT:", 500ul);
}
...
void exec(const char* topic, const char* payl) { //++++++++++++++  USER COMMAND IMPLEMENTAION  ++++++++++++++++++
  ...
  if        (!strcmp(topic, "wutm"))  {   //*************** warmup timer **************
    ...
    else               ign.set(false), warmup = false, _delay_ms(250);
    ...
    modem.print(F("AT+BTPOWER=")); modem.println(_pw ? "1" : "0"); waitresp(modem, NULL, 3900ul);
    wdt_reset();
    if (_pw) {
      modem.println(F("AT+BTVIS=0"));
      if (!waitresp(modem, "OK\r\n")) modem.println(F("AT+BTPOWER=0")), waitresp(modem, NULL, 2900ul);
    }
    wdt_reset();
...
void btspp(const char* inp, byte len) { //+++++++++++++++++++ BLUETOOTH SETUP THROUGH SPP  ++++++++++++++++++
  ...
  if        ( !strncmp(inp, "srv", 3) && (_paramlen < 0x20) ) {
    ...
    modem.println(F("AT+BTSPPSEND")); if (waitresp(modem, "> ")) modem.println(F(" ok")), modem.write(0x1A), waitresp(modem);
  } else if ( !strncmp(inp, "usr", 3) && (_paramlen < 0x10) ) {
    broker(_paramptr, _paramlen);
    //eeprom_update_block(_paramptr, 0x20, _paramlen); eeprom_update_byte(0x20+_paramlen, 0); eepage = 0;
    modem.println(F("AT+BTSPPSEND")); if (waitresp(modem, "> ")) modem.println(F(" ok")), modem.write(0x1A), waitresp(modem);
  } else if ( !strncmp(inp, "pwd", 3) && (_paramlen < 0x10) ) {
    pass(_paramptr, _paramlen);
    //eeprom_update_block(_paramptr, 0x30, _paramlen); eeprom_update_byte(0x30+_paramlen, 0); eepage = 0;
    modem.println(F("AT+BTSPPSEND")); if (waitresp(modem, "> ")) modem.println(F(" ok")), modem.write(0x1A), waitresp(modem);
    ...
    if ( (_idx >= PCB) && (_idx <= VEH) ) {
      ...
      if (_found) {
        sid(_idx, _sid);
        //eeprom_update_block(_sid, 0x40 + 8*_idx, 8); eepage = 0;//EEPROM.put(0x40 + 8*_idx, _sid);    // sensor id to eeprom
      ...
      modem.println(F("AT+BTSPPSEND")); if (waitresp(modem, "> ")) {
        ...
        modem.println(); modem.write(0x1A), waitresp(modem);
      }
    } else {
      modem.println(F("AT+BTSPPSEND")); if (waitresp(modem, "> ")) {
        modem.println(F("\r\nstored:"));
    ...
    modem.println(F("AT+BTSPPSEND")); if (waitresp(modem, "> ")) modem.println(F(" reconnect with new pin!")), modem.write(0x1A), waitresp(modem);
    modem.println(F("AT+BTUNPAIR=0")); waitresp(modem);
    modem.print(F("AT+BTPAIRCFG=1,")), modem.println(_paramptr); waitresp(modem);
  } else if (!strncmp(inp, "moto", 4)) {                            // ======== running hours ============
    uint16_t _min = atoi(_paramptr) * 60;
    modem.println(F("AT+BTSPPSEND")); if (waitresp(modem, "> ")) modem.println((moto(_min) == _min) ? F(" ok") : F(" error")), modem.write(0x1A), waitresp(modem);
  } else if (inp[0] == '?') {
    modem.println(F("AT+BTSPPSEND")); if (waitresp(modem, "> ")) {
      modem.println();
    ...
    modem.println(F("AT+BTVIS=0")); waitresp(modem);
    modem.println(F("AT+BTSPPSEND")); if (waitresp(modem, "> ")) modem.println(F(" setup over!")), modem.write(0x1A), waitresp(modem);
    eepage = 0; setupmode = false; led.set(false);
  } else {
    modem.println(F("AT+BTSPPSEND")); if (waitresp(modem, "> ")) modem.println(F(" error!")), modem.write(0x1A), waitresp(modem);
  }
}
void dtmf(const char key, bool lon) { //+++++++++++++++++++ ONCALL KEYPRESS HANDLING  ++++++++++++++++++
  ...
  if ( ((key == '#') || (key == '0')) && lon ) {                                                    // emergency recovery
    wdt_disable();
    if (callstate == 0x0C) play((key == '#') ? "recov" : "reboot"), waitresp(modem, "+CREC: 0", 5000ul);
    modem.println(F("ATH")); waitresp(modem);
    ...
    if (setupmode) {
      modem.println(F("AT+BTVIS=0")); waitresp(modem);
      eepage = 0; setupmode = false;
    } else {
      modem.println(F("AT+BTPOWER=1")); wdt_reset();
      waitresp(modem, NULL, 3900ul);    wdt_reset();
...
//  if (!modem.available()) return;
  ...
  _ptr = strstr(at, "+BTCONNECT:"); if (_ptr)
    if  (setupmode && strstr(_ptr+12, "\"SPP\"")) { 
      modem.println(F("AT+BTSPPGET=0")); waitresp(modem);
      modem.println(F("AT+BTSPPSEND"));
      if (waitresp(modem, "> ")) modem.println(F("Welcome to BlackBox setup!")), modem.write(0x1A), waitresp(modem);
    } else {
...
//  if (strstr(at, "BTRING") && btansw) if ((millis() - tbtring) > 13000ul) tbtring = millis(); else if ((millis() - tbtring) > 8000ul) tbtring = 0, modem.println(F("AT+BTATA")); // with 3rd ring answer
  ...
  _ptr = strstr(at, "+CPBF:");  if (_ptr) {                                             // recv ph.num "admin" then send sms CHECK!
    char* _nm = strchr(_ptr+6, '"'); if (_nm) {
      ...
      if (waitresp(modem, "> ")) modem.write(msg), modem.write(0x1A);
    }
  }
...
void loop() {
  ...
  if (starter.active()) ign.set(false);
  if (setupmode) {
    if (EIFR) {
      _delay_ms(100);
      modem.println(F("AT+BTSPPSEND"));
      if (waitresp(modem, "> ")) modem.println(EIFR & 1 ? F(" ! (>_<) !") : F(" (-_-)")), modem.write(0x1A), waitresp(modem);
      EIFR = EIFR;
    }
    return;
  }
