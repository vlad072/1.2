...
void starting() {
  ...
  for (uint32_t _t = millis(); (millis() - _t) < 2000ul;) if (vbatt.value() > _vrelease) break; // spinup engine - max 3.0sec
...
//  }
  ds.reset(); ds.write(0xCC); ds.write(0x4E); ds.write(0x7F), ds.write(0xFF); ds.write(0x1F);  // check
  ds.reset(); ds.write(0xCC); ds.write(0x48);
}
void dsupdate() {
...
void loop() {
  ...
  if ( (celstate == 0) && keepconn && ((millis() - tresp) > 60000ul) ) modemshut(); // broker isn't responding during 1min!
  ...
  if ( (alarm & ALARMS) && (celstate == 0) )
    if ( (((millis() - tlastcall) > trecall) && (trecall < 7680001ul)) || (alarm & NEW_ALARM) ) {
      modem.println(F("AT+DDET=1,500,0;D>\"admin\";")); tlastcall = millis();
      if (modem.find("OK\r\n")) celstate = ALERT, trecall = (trecall == 5000ul) ? 120000ul : trecall*4; // recall intervals: 2,8,32,128 min
      else                                        trecall = 5000ul;                                     // when modem fail trying every 5sec
  }
  if (alarm & NEW_ALARM) {
    alarm &= ~NEW_ALARM; dvr.twitch(60000ul);
    if (trecall != 5000ul) trecall = 120000ul;
    if (celstate < INCALL) {
