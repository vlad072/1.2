...
void starting() {
  ...
  wdt_reset();
  #endif
  float _vrelease = vbatt.value() - 1.7f; //uint32_t _tinit = millis(); //_tmout = temps[CUR][ENG] == -127 ? 1500 : constrain( 1200 - temps[CUR][ENG] * 16, 700, 2000 );
  starter.set(true); delay(500); if (!starter.active()) goto _err;
  for (uint32_t _t = millis(); (millis() - _t) < 2000ul;) if (vbatt.value() > _vrelease) break;
...
//    if (sendstart()) pub("notify", "broker isn't responding!"), sendfin();
  ...
  if ((alarm & ALARMS) && (celstate == 0) && !modem.available())
    if ( ((millis() - tlastcall > trecall) && (trecall < 7680001ul)) || (alarm & NEW_ALARM) )
      modem.println(F("AT+DDET=1,500,0;D>\"admin\";")), celstate = ALERT, tlastcall = millis(), trecall *= 4; // recall periods: 2,8,32,128 min
  if (alarm & NEW_ALARM) {
    alarm &= ~NEW_ALARM; trecall = 120000ul; dvr.twitch(60000ul);
    if (celstate < INCALL) {
