...
void starting() {
  ...
  uint32_t _rotate = 750ul; if (temps[CUR][ENG] < 0) _rotate -= temps[CUR][ENG]*10;
...
bool sendstart() {
  ...
  return waitresp("> ", 300ul);
}
bool sendfin() {
  modem.write(0x1A);
  return waitresp("DATA ACCEPT:", 500ul);
}
void pub(const char* topic, const char* payl, byte retn = 0) {
  byte _tlen = strlen(pref()) + 4 + strlen(topic);
...
void pubn(const char* topic, float val, byte dap = 0) {
  char _st[8] = "";
  dtostrf(val, 0, dap, _st); pub(topic, _st);
}
void sub() {
  byte _tlen = strlen(pref()) + 5;                //pref+cmd/#
...
void brokercon () {
  ...
  if (!sendstart()) return;
  modem.write(0x10);
  modem.write( 4 + 2+strlen(proto) + 2+strlen(cid) + 2+strlen(pref())+4+strlen(willt) + 2+1 + 2+strlen(user()) + 2+strlen(pass()) );
  modem.write((byte)0x00); modem.write(strlen(proto));  modem.print(proto); modem.write(pver);
  ...
  modem.write((byte)0x00); modem.write(strlen(pref())+4+strlen(willt)); modem.print(pref()); modem.print("inf/"); modem.print(willt);  // will topic
...
void locking(bool lk) {
  ...
  if (lk) {                   // lock
    ...
    if (secure & 0x04)
      ...
      else if (alarm & 0x01)  siren.twitch(50ul,  2);
      else                    siren.twitch(1ul);
    unlock.twitch(300ul); flash.twitch(350ul);
    secure &= ~0x80;
  }
//  if (sendstart()) pubn("inf/secure", secure), sendfin();
}
void exec(const char* topic, const char* payl) { //++++++++++++++  USER COMMAND IMPLEMENTAION  ++++++++++++++++++
  ...
  if        (!strcmp(topic, "wutm"))  {   //*************** warmup timer **************
    ...
    switch (payl[0]) {
      case '1': alarm = 0;                                                                // clear
      case '2': siren.set(0); flash.set(0); break;                                        // mute
      case '3': if (secure & 0x04) siren.twitch(500ul, 3); flash.twitch(3000ul); return;  // raise
    }
...
void ipd( const byte* frame, byte len = 1 ) { //++++++++++++++  PROCESSING DATA FROM BROKER  ++++++++++++++++++
  for (byte _ofs = 0; _ofs < len;) {
    if ( (frame[_ofs] == 0xD0) && (frame[_ofs+1] == 0x00) ) {            // pingresp
      ...
      if (sendstart()) pub(willt, "1", RETAIN), sendfin();
      dbg("suback");
    } else if (frame[_ofs] == 0x30) {                                    // pub recv
      char* _tptr = (char*)(frame + _ofs + 4 + strlen(pref())+4);
      byte  _tlen = frame[_ofs+3] - strlen(pref())-4;
...
void btspp(const char* frame, byte len) { //+++++++++++++++++++ BLUETOOTH SETUP THROUGH SPP  ++++++++++++++++++
  char* _paramptr = strchr(frame, '=') + 1; byte _paramlen = constrain( len-(_paramptr-frame), 0, PAGESZ-1 );
  if        (!strncmp(frame, "srv", 3)) {
    ...
    modem.println(F("AT+BTSPPSEND")); if (waitresp("> ")) modem.println(strncmp(user(), _paramptr, _paramlen) ? F(" fail!") : F(" ok")), modem.write(0x1A), waitresp();
  } else if (!strncmp(frame, "pwd", 3)) {
    pass(_paramptr, _paramlen);
    modem.println(F("AT+BTSPPSEND")); if (waitresp("> ")) modem.println(strncmp(pass(), _paramptr, _paramlen) ? F(" fail!") : F(" ok")), modem.write(0x1A), waitresp();
  } else if (!strncmp(frame, "pref", 4)) {
    pref(_paramptr, _paramlen);
    modem.println(F("AT+BTSPPSEND")); if (waitresp("> ")) modem.println(strncmp(pref(), _paramptr, _paramlen) ? F(" fail!") : F(" ok")), modem.write(0x1A), waitresp();
    ...
    moto(WRITE_MOTO, atoi(_paramptr)*60);
    modem.println(F("AT+BTSPPSEND")); if (waitresp("> ")) modem.println( (moto() == atoi(_paramptr)*60) ? F(" ok") : F(" fail!")), modem.write(0x1A), waitresp();
  } else if (frame[0] == '?') {
    modem.println(F("AT+BTSPPSEND")); if (waitresp("> ")) {
      modem.println();
...
//  if (!modem.available()) return;
  ...
  if (strstr(at, "+BTDISCONN:")) {
    if (!(secure & 0x80) && !pump.active() && !setupmode) {
      ...
      locking(1);
      if (sendstart()) pubn("secure", secure), sendfin();      
      notify(_op ? "VEH ISN'T CLOSED!" : "BT ARMED", _op ? PUSH+SMS : PUSH);
    }
    modem.println(F("AT+BTSTATUS?"));
  }
  ...
  _ptr = strstr(at, "+BTSTATUS:");  if (_ptr) {
    ...
    if (sendstart()) pubn("btst", _st), pub(willt, "1"), sendfin();
  }
...
void loop() {
  ...
  if (dvrmode & 0x08) if (vbatt.value() > 11.7f) dvr.twitch(5000ul);
  if (pump.active()) {
    tengrun = millis();
  ...
  alarm &= ~0xC0;
  lock.processing(); unlock.processing(); dvr.processing(); siren.processing(); flash.processing(); led.processing();
} // ************* LOOP +++++++++++++++++++++++
