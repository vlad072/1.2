...
void loop() {
  ...
  if (keepconn) {
    if ((millis() - tsend) > 27333ul) ping(), tsend = millis();
    if ((millis() - tresp) > 90000ul) if (celstate == 0) modemshut(); // broker isn't responding during 1min!
  }
...
//    if (smsalert) alarm &= ~SHOCK1;
  ...
  if ( (alarm & ALARMS) && (celstate == (INCALL + VOICE_RDY)) )
    for ( byte _i = 5; _i > 0; _i-- ) {                       //  0( h01 ) = shock1 
      ...
      if ((1 << _i) & FIRE_DET)  { play("fire");     break; } //  3( h08 ) = door open   // CHECK!                           
