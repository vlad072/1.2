...
void starting() {
  ...
  if (starter.active()) { ign.set(false); return; }                         // mosfet breakdown
  delay(3000); wdt_reset();
  float _vrelease = vbatt.value() - 1.7f; //uint32_t _tinit = millis();
  ...
  wdt_reset(); starter.set(false); delay(100); if (starter.active()) { ign.set(false); return; }
  delay(3000); wdt_reset();
  warmup = engrun();
...
void modemshut() {
  ...
  tsend = tresp = millis(); tping = DEF_PING;
}
void play(const char* track) {
  if (celstate != (INCALL + VOICE_RDY)) return; dbg("play", track);
  celstate &= ~VOICE_RDY;
...
void brokercon () {
  ...
  modem.write(0xE6); modem.write((byte)0x00); modem.write((byte)(DEF_PING/1000+9));             // flags, ka timout | E/C -retain/not
...
void locking(bool lk) {
  ...
  if (lk) {                   // lock
    ...
    if (_opened) siren.twitch(100ul, 3);
    lock.twitch(500ul, 2); flash.twitch(1200ul);
    secure |= ARMED | SHOCK_ON;
    door.change(); hood.change(); EIFR = EIFR; alarm = 0;
  } else {                    // unlock
    if (secure & SIREN_ON)
      if      (alarm & ALARMS & ~SHOCK2) siren.twitch(50ul, 3);
      else if (alarm & SHOCK2)           siren.twitch(50ul, 2);
      else if (alarm & SHOCK1)           siren.twitch(50ul, 1);
    notify("DISARMED");
...
void dtmf(const char cmd) { //+++++++++++++++++++ ONCALL KEYPRESS HANDLING  ++++++++++++++++++
  ...
  if (cmd == '#') {                                                             // emergency recovery
    wdt_disable();
    if (celstate == (INCALL + VOICE_RDY)) modem.setTimeout(5000), play("recov"), modem.find("+CREC: 0");
...
//  if (!modem.available()) return;
  ...
  _ptr = strstr(at, "+CLCC: 1,"); if (_ptr) {
    switch (_ptr[11]) {
      case '0': siren.set(false); flash.set(false); celstate = (INCALL + VOICE_RDY); play("hello"); break;
      case '2':
      case '3': celstate = ALERT; break;
      case '6':
      case '7': celstate = 0; tresp = millis() - DEF_PING; break;
      case '4': modem.println(strstr(_ptr+17, "\"admin\"") ? F("AT+DDET=1,500,0;A") : F("AT+CHUP"));
    } t20sec = millis();
  }
  if (strstr(at, "+CREC: 0")) celstate |= VOICE_RDY;
  if (strstr(at, "+CPAS: 0")) celstate  = 0; tresp = millis() - DEF_PING;
...
//  if (strstr(at, "BTRING") && btansw) if ((millis() - tbtring) > 13000ul) tbtring = millis(); else if ((millis() - tbtring) > 8000ul) tbtring = 0, modem.println(F("AT+BTATA")); // with 3rd ring answer
  ...
  _ptr = strstr(at, "+IPD");        if (_ptr) tresp = millis(), tping = DEF_PING, ipd( strchr(_ptr + 6, ':') + 1, atoi(_ptr + 5) );
...
void dsupdate() {
  ...
  if (secure & ARMED)                                                                                  // overheats:
    if ( (temps[DIF][PCB]  > 2) || (temps[CUR][PCB]  > 60) ||                                          //  pcb
         (temps[DIF][OUTS] > ((millis() - tengrun) < 180000ul ? 6 : 2)) || (temps[CUR][OUTS] > 60) ||  //  outside
         (temps[DIF][VEH]  > ((millis() - tengrun) < 300000ul ? 4 : 2)) || (temps[CUR][VEH]  > 75) )   //  vehicle
...
void setup() {
  ...
  dsinit(); dsupdate();
  if (neutral()) secure = ARMED | SHOCK_ON | SIREN_ON;
  #if DEBUG_LVL
...
void loop() {
  ...
  if (keepconn) {
    tping = ((millis() - tresp) > (DEF_PING + 4000ul)) ? 3000ul : DEF_PING;
    if (celstate == 0) {
      if ((millis() - tsend) > tping) ping(), tsend = millis();
      if ((millis() - tresp) > (DEF_PING + 10000ul)) modemshut(); // broker isn't responding!
    }
  }
  ...
  if (warmup) {
    if ((millis() - t1min) > 90000ul) { t1min = millis(); if (warmtimer > 0) warmtimer--; }
    warmup = pump.active() && neutral() && (warmtemp > temps[CUR][ENG]) && (warmtimer > 0);   
    if (!warmup) ign.set(false);
  }
  if (secure & ARMED) {
    if (EIFR) _delay_ms(100), alarm |= (EIFR & 0x01) ? (NEW_ALARM | SHOCK2) : (((secure & SHOCK_ON) && !engrun()) ? (NEW_WARN | SHOCK1) : 0), EIFR = EIFR; // CHECK
    if (hood.change() == 1) alarm |= NEW_ALARM | HOOD_OPEN;
    if (door.change() == 1) alarm |= NEW_ALARM | DOOR_OPEN;
    if (pump.change() == 1) alarm |= NEW_ALARM | IGN_ON;
  }
  if (alarm & NEW_ALARM) {
  ...
  if ( (alarm & ALARMS) && (celstate == 0) )
    if ( ((nrecall > 0) && ((millis() - tlastcall) > 120000ul)) || (alarm & NEW_ALARM) ) {
      modem.println(F("AT+DDET=1,500,0;D>\"admin\";")), tlastcall = millis(), nrecall--;
      if (modem.find("OK\r\n")) celstate = ALERT;
  }
  if (alarm & NEW_WARN) {
    dvr.twitch(30000ul);
    if ((millis() - tlastmsg) > (smsnotify ? 5000ul : 1000ul)) notify("BANG! (>_<)", smsnotify ? SMS : PUSH+SMS), tlastmsg = millis();
//    if (smsalert) alarm &= ~SHOCK1;
    if (!siren.on() && (secure & SIREN_ON)) siren.twitch(50ul, 5);
    if (!flash.on()) flash.twitch(2000ul);
  }
  if ( (alarm & ALARMS) && (celstate == (INCALL + VOICE_RDY)) )
    for ( byte _i = 5; _i > 0; _i-- ) {                       //  0( h01 ) = shock1 
      if (!bitRead(alarm, _i)) continue;                      //  1( h02 ) = shock2
      bitClear(alarm, _i);                                    //  2( h04 ) = hood open
      if ((1 << _i) & FIRE_DET)  { play("fire");     break; } //  3( h08 ) = door open
      if ((1 << _i) & IGN_ON)    { play("ignon");    break; } //  4( h10 ) = ignition on
      if ((1 << _i) & DOOR_OPEN) { play("dooropen"); break; } //  5( h20 ) = fire detect
      if ((1 << _i) & HOOD_OPEN) { play("hoodopen"); break; } //  6( h40 ) = NEW WARNING
      if ((1 << _i) & SHOCK2)    { play("shock");    break; } //  7( h80 ) = NEW ALARM
  }
  alarm &= ~NEW_ALARM & ~NEW_WARN;
