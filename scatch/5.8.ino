...
void starting() {
  ...
  uint32_t _rotate = 750ul; int8_t _tc = 0;
...
void play(const char* track) {
  if (callstate < 0x04) return; dbg("play", track);
  stoptrack();
...
void locking(bool lk) {
  if (setupmode) return;
  if (lk) {                   // lock
    if (door.active() || hood.active()) siren.twitch(100ul, 3); else if (secure & 0x04) siren.set(1), _delay_ms(15), siren.set(0);
    ...
    if (secure & 0x04)
      ...
      else                    siren.set(1), _delay_ms(15), siren.set(0);
...
void btspp(const char* frame, byte len) { //+++++++++++++++++++ BLUETOOTH SETUP THROUGH SPP  ++++++++++++++++++
  ...
  if        (!strncmp(frame, "srv", 3)) {
    ...
    byte _sid[8] = {0};  byte _idx = _paramptr[0] - '0';
    ...
    modem.println(F("AT+BTSPPSEND")); if (waitresp("> ")) modem.println( (moto() == atoi(_m)*60) ? F(" ok") : F(" fail!")), modem.write(0x1A), waitresp();
  } else if (frame[0] == '?') {
    modem.println(F("AT+BTSPPSEND")); if (waitresp("> ")) {
      modem.println();
...
void dtmf(const char key, bool lon) { //+++++++++++++++++++ ONCALL KEYPRESS HANDLING  ++++++++++++++++++
  ...
  if ( ((key == '#') || (key == '0')) && lon ) {                               // emergency recovery
    ...
    secure ^= 0x01; play(secure & 0x01 ? "shk1" : "shk0");
    ...
    if (pump.active()) ign.set(0); else starting();
    _delay_ms(500);
...
void loop() {
  ...
  if (secure & 0x80) {
    ...
    if (batt < lowbatt) {
      strcpy(msg, "LOW BATT: "); dtostrf(batt, 5, 2, msg+10); notify(NULL, PUSH+SMS);
      lowbatt -= 0.25f;
    } else {
