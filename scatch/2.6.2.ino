...
void loop() {
  ...
  if (keepconn) {
    ...
    if ((millis() - tresp) > 60000ul) if (celstate == 0) modemshut(); // broker isn't responding during 1min!
  }
  ...
  if (warmup) {
    if ((millis() - t1min) > 60000ul) { t1min = millis(); if (warmtimer > 0) warmtimer--; }
    warmup = pump.active() && neutral() && (warmtemp > temps[CUR][ENG]) && (warmtimer > 0);   
    if (!warmup) ign.set(false);
  }
  if (secure & ARMED) {
    if (secure & SHOCK_ON) if (EIFR) _delay_ms(100), alarm |= EIFR & 0x01 ? (NEW_ALARM | SHOCK2) : (engrun() ? 0 : (NEW_WARN | SHOCK1)), EIFR = EIFR;
  ...
  if (alarm & NEW_WARN) {
    dvr.twitch(30000ul);
    if ((millis() - tlastmsg) > (smsnotify ? 5000ul : 1000ul)) notify("BANG! (>_<)", smsnotify ? SMS : PUSH), tlastmsg = millis();
