...
const char* fwver   = "6.0";       // firmware version
//===================== variables ======================
...
byte     callstate  =  0;       // undef=0, initial=1, alert=2, in_talk=4, speak=8 /// play_ready=0x04
int8_t   warmtimer  =  20;      // engine warming count-down timer min 
int8_t   warmtemp   =  50;      // engine warming temperature limit deg.C 
byte     secure     =  0;       // bits: 7(80)=lock, 6(40) = siren active, 5(20) = fire detect, 1(2)= shock high, 0(1)=shock low
bool     smsnotify  =  0;       // notifications through SMS
byte     alarm      =  0;       // alarm flags
byte     alarmcash  =  0;       // alarm cash
...
byte eebuf[PAGESZ] = {0}; // eeprom map buffer
