...
const char* infpref = "inf/";        // outgoing topics prefix
const char* cmdpref = "cmd/";        // incoming topics prefix
const char* fwver   = "5.3";         // firmware version
//===================== variables ======================
...
char* broker(char* str = NULL, byte len = 0) {                            //  0..1F  ===== page 1 =====
  if (str) {
...
char* user(char* str = NULL, byte len = 0) {                              // 20..2F  ===== page 2 =====
  if (str) {
    if (len == 0) len = strlen(str);
    eeprom_update_block(str, 0x20, len); eeprom_update_byte(0x20+len, 0); eepage = 0;
  } else {
    if (eepage != 2) eeprom_read_block(rombuf, 0x20, 0x20), eepage = 2;
    return rombuf;
  }
}
char* pass(char* str = NULL, byte len = 0) {                              // 30..3F
  if (str) {
    if (len == 0) len = strlen(str);
    eeprom_update_block(str, 0x30, len); eeprom_update_byte(0x30+len, 0); eepage = 0;
  } else {  
    return user() + 0x10;
  } 
}
byte* sid(byte idx, byte* id = NULL) {                                    // 40..5F  ===== page 3 =====
  if (id) {
    eeprom_update_block(id, 0x40 + idx*8, 8); eepage = 0;
  } else {
...
uint16_t moto(byte act = READ_MOTO, uint16_t val = 1) {                    // 80..9F  ==== moto min ==== (invert!)
  ...
  switch (act) {
    case ADD_MOTO  : if (_raw < 0xFFFF) _raw += val, eeprom_update_word(0x80+_offs, ~_raw);
                     break; //return ~eeprom_read_word(0x80+_offs);
