...
uint8_t  recmode    =  0x03;    // recording dvr 1 = drive, 2 = parking, 3 = both
...
uint8_t  secure     =  0;       // bits: 7(80)=lock, 0(1)=shock low, 1(2)= shock high, 2(4) = siren active
...
char* broker(char* val = NULL, size_t len = 0) {                            //  0..1F  ===== page 1 =====
  if (val && len) {
...
char* user(char* val = NULL, size_t len = 0) {                              // 20..2F  ===== page 2 =====
  if (val && len) {
...
char* pass(char* val = NULL, size_t len = 0) {                              // 30..3F
  if (val && len) {
...
uint16_t moto(uint16_t val = READ_MOTO) {   // 80..9F  ==== moto min ==== (invert bits)
  ...
  switch (val) {
    ...
    default       : eeprom_update_word(0x80+_offs, 0xFFFF);
                    _offs = (_offs < 0x1C) ? _offs+2 : 0;
