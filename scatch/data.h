...
uint8_t  callstate  =  0;       // undef=0, initial=1, alert=2, in_talk=4, channel_free=8 /// play_ready=0C
int8_t   warmtimer  =  20;      // engine warming count-down timer min 
int8_t   warmtemp   =  50;      // engine warming temperature limit deg.C 
uint8_t  secure     =  0;       // bits: 7(80)=lock, 0(1)=shock sensor enabled, 1(2)=siren active
...
uint16_t moto(uint16_t val = READ_MOTO) {   // 60..7F  ==== moto min ==== (invert bits)
  byte _offs = ~eeprom_read_byte(0x60); uint16_t _raw = ~eeprom_read_word(0x62+_offs);
  switch (val) {
    case INC_MOTO : if (_raw < 0xFFFF) _raw++; eeprom_update_word(0x62+_offs, ~_raw);
    case READ_MOTO: return _raw;
    default       : eeprom_update_word(0x62+_offs, 0xFFFF);
                    _offs = (_offs < 0x1E) ? _offs+2 : 0;
                    eeprom_update_byte(0x60, ~_offs);
                    eeprom_update_word(0x62+_offs, ~val);
  }
}
//=================== tracks ==========================
