...
void starting() {
  ...
  starter.set(1); delay((temps[CUR][ENG] > 0) ? 750ul : 850ul);
...
void pub(const char* topic, const char* payl, byte retn = 0) {
  byte _topiclen = strlen(infpref) + strlen(topic);
  modem.write(0x30 | retn);
  modem.write(2+ _topiclen + strlen(payl));
  modem.write((byte)0x00); modem.write(_topiclen);
  modem.write(infpref); modem.write(topic); modem.write(payl);
}
...
void brokercon () {
  ...
  if (!sendstart()) return;
  modem.write(0x10);
  modem.write( 4 + 2+strlen(proto) + 2+strlen(cid) + 2+strlen(infpref)+strlen(willt) + 2+1 + 2+strlen(user()) + 2+strlen(pass()) );
  modem.write((byte)0x00); modem.write(strlen(proto));  modem.write(proto); modem.write(pver);
  ...
  modem.write((byte)0x00); modem.write(strlen(infpref)+strlen(willt)); modem.write(infpref); modem.write(willt);  // will topic
  ...
  for (byte _ofs = 0; _ofs < len;) {
    if ( (frame[_ofs] == 0xD0) && (frame[_ofs+1] == 0x00) ) {            // pingresp
      ...
      if (frame[_ofs+3] == 0x00) {
        char _sub[strlen(cmdpref)+2]; strcpy(_sub, cmdpref); strcat(_sub, "#");
        if (sendstart()) pub(willt, "1", RETAIN), sub(_sub), sendfin();
      ...
      char* _tptr = (char*)(frame + _ofs + 4 + strlen(cmdpref));
      byte  _tlen = frame[_ofs+3] - strlen(cmdpref);
