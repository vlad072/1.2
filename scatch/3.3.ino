...
void exec(const char* topic, const char* payl) { //++++++++++++++  USER COMMAND IMPLEMENTAION  ++++++++++++++++++
  ...
  if        (!strcmp(topic, "wutm"))  {   //*************** warmup timer **************
    ...
    else               ign.set(false), warmup = false, _delay_ms(500);
...
void btspp(const char* inp, size_t len) { //+++++++++++++++++++ BLUETOOTH SETUP THROUGH SPP  ++++++++++++++++++
...
//  if (!modem.available()) return;
  ...
  _ptr = strstr(at, "+BTSTATUS:");  if (_ptr) {
    ...
    byte _st =( _pair & 0x07) | ((_conn & 0x07) << 3) | (btansw << 6) | ((_ptr[11] > '0') << 7); //bits: WACCCPPP (W=power, A=autoansw, C=connected, P=paired)
...
void loop() {
  ...
  if ( pump.active() && !warmup && !neutral() && bitRead(recmode, 0) ) dvr.twitch(10000ul);
