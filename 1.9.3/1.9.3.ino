...
void cmd(const char* topic, const char* payl) { //++++++++++++++  USER COMMAND IMPLEMENTAION  ++++++++++++++++++
  ...
  if        (!strcmp(topic, "wutm"))  {   //*************** warmup timer **************
    ...
    if (payl[0] > '0')
      if (secure & ARMED) secure |= SHOCK_ON, EIFR = EIFR;
      else               locking(true);
...
//  if (!modem.available()) return;
  ...
  _ptr = strstr(at, "+BTCONNECT:"); if (_ptr)
    ...
    else if ( (secure & ARMED) && (_ptr[12] > '0') && (!engrun() || warmup) ) locking(false);
  if    (strstr(at, "+BTDISCONN:")) if (!(secure & ARMED) && !pump.active()) locking(true);
...
//  if (tinitlock) if ((millis() - tinitlock) > 30000ul) tinitlock = 0, locking(true);
  ...
  if (secure & ARMED) {
    if (secure & SHOCK_ON) if (EIFR) _delay_ms(70), alarm |= EIFR & 0x01 ? 0x82 : (engrun() ? 0x00 : 0x41), EIFR = EIFR;
  ...
  if (alarm & NEW_WARN) {
    ...
    if (!siren.on() && (secure & SIREN_ON)) siren.twitch(200ul, 2);
