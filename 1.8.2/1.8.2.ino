...
bool engrun() {
  return pump.active() || (vbatt.value() > 13.3f);
}
bool drive() {
...
void modemreconf() {
  ...
  modem.println(F("AT+CMIC=0,13;+BTPAIRCFG=1;+CLCC=1;E0&W"));   modem.find("OK\r\n"); // modem.find("OK\r\n");
}
...
//  if (tinitlock) if ((millis() - tinitlock) > 30000ul) tinitlock = 0, locking(true);
  ...
  if (bitRead(locked, 7)) {
    if (bitRead(locked, 0)) if (EIFR) _delay_ms(100), alarm |= EIFR & 0x01 ? 0x82 : 0x41, EIFR = EIFR;
    ...
    if (!bitRead(alarm, 5)) if ( (temps[DIF][PCB]  > 2)                  || (temps[CUR][PCB]  > 60) ||  //   pcb
                                 (temps[DIF][OUTS] > 2)                  || (temps[CUR][OUTS] > 60) ||  //  outside
                                 (temps[DIF][VEH]  > (engrun() ? 4 : 2)) || (temps[CUR][VEH]  > 75) )   //  vehicle
                            alarm |= bit(7) | bit(5), ign.set(false);
  }
  if (bitRead(alarm, 7)) {
    dvr.run(60000ul);
    if (celstate == 0) modem.println(F("ATD>\"admin\";")), bitClear(alarm, 7);
    if (celstate <  2) {
  ...
  if (bitRead(alarm, 6)) {
    ...
    if (smsalert) modem.println(F("AT+CPBF=\"admin\"")), alarm &= ~0x01;    // request ph.num admin
