...
void pubalarm() {
  char _sign[12] = "F I D H S B";
  for (byte _i = 0; _i < 6; _i++) if (alarm ^ bit(_i)) _sign[10-_i*2] = '-';
  pub("inf/alarm", _sign);
}
...
//  if (tinitlock) if ((millis() - tinitlock) > 30000ul) tinitlock = 0, locking(true);
  ...
  if (locked & 0x80) {
    ...
    if (hood.change()     == 1) alarm |= 0x80 || 0x04;
    if (door.change()     == 1) alarm |= 0x80 || 0x08;
    if (fuelpump.change() == 1) alarm |= 0x80 || 0x10;
  } if (!(alarm & 0x20)) if  ( (temps[DIF][PCB] > 5) || (temps[CUR][PCB] > 70) || (!engrun() && ((temps[DIF][OUTS] > 3) || (temps[DIF][VEH] > 3))) ) ign.set(false), alarm |= 0x80 || 0x20;
  if         (alarm & 0x80) {
